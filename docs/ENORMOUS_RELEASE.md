...
> Research release by owner decision (2026-09-25). ENORMOUS **failed** its preregistered rule-reading gate; no generalization claim is made. `{HF_REVISION}` must be filled with the published adapter commit, and `ENORMOUS_REVISION` pinned in `src/nagi/enormous.py`; until then `load_enormous()` raises instead of loading.
